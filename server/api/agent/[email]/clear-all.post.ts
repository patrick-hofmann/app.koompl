...
export default defineEventHandler(async (event) => {
  ...
  const emailsResult = await clearAgentEmails({ agentId: agent.id })
  ...
  const totalDeleted =
    emailsResult.deletedCount + mailLogsResult.deletedCount + activityLogsResult.deletedCount

  return {
    ok: true,
    message: `Cleared ${totalDeleted} items for agent ${email}`,
