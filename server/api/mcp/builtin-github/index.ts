...
import { githubDefinition } from './definition'
...
export default defineEventHandler(async (event) => {
  ...
  if (method === 'GET') {
    return {
      ok: true,
      message: 'Builtin GitHub MCP endpoint. Send JSON-RPC 2.0 requests via POST.'
    }
  }
  ...
  console.log('[BuiltinGithubMCP] Authenticated:', {
    teamId,
    userId,
  ...
  const baseContext: GithubMcpContext = {
    teamId,
    userId
  }
  ...
  try {
    ...
    switch (body.method) {
      case 'initialize': {
        result = {
          ...
          capabilities: { tools: {} },
          serverInfo: { name: 'builtin-github-server', version: '1.0.0' }
        }
        break
      }
      ...
      case 'tools/list': {
        result = {
          tools: githubDefinition.tools.map((tool) => ({
            name: tool.name,
      ...
      case 'tools/call': {
        ...
        const tool = githubDefinition.tools.find((t) => t.name === toolName)
        if (!tool) {
    ...
    console.error('[BuiltinGithubMCP] Error handling request:', error)
    return {
      jsonrpc: '2.0',
      id: requestId,
      error: {
