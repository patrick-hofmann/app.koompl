...
export default defineEventHandler(async (event) => {
  ...
  if (method === 'GET') {
    return {
      ok: true,
      message: 'Builtin Analytics MCP endpoint. Send JSON-RPC 2.0 requests via POST.'
    }
  }
  ...
  console.log('[BuiltinAnalyticsMCP] Authenticated:', {
    teamId,
    userId,
  ...
  const baseContext: AnalyticsMcpContext = {
    teamId,
    userId
  }
  ...
  try {
    ...
    switch (body.method) {
      case 'initialize': {
        result = {
          ...
          capabilities: { tools: {} },
          serverInfo: { name: 'builtin-analytics-server', version: '1.0.0' }
        }
        break
      }
      ...
      case 'tools/list': {
        result = {
          tools: analyticsDefinition.tools.map((tool) => ({
            name: tool.name,
      ...
      case 'tools/call': {
        ...
        const tool = analyticsDefinition.tools.find((t) => t.name === toolName)
        if (!tool) {
    ...
    console.error('[BuiltinAnalyticsMCP] Error handling request:', error)
    return {
      jsonrpc: '2.0',
      id: requestId,
      error: {
