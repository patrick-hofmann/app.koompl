...
export default defineEventHandler(async (event) => {
  ...
  if (method === 'GET') {
    return {
      ok: true,
      message: 'Builtin Monitoring MCP endpoint. Send JSON-RPC 2.0 requests via POST.'
    }
  }
  ...
  console.log('[BuiltinMonitoringMCP] Authenticated:', {
    teamId,
    userId,
  ...
  const baseContext: MonitoringMcpContext = {
    teamId,
    userId
  }
  ...
  try {
    ...
    switch (body.method) {
      case 'initialize': {
        result = {
          ...
          capabilities: { tools: {} },
          serverInfo: { name: 'builtin-monitoring-server', version: '1.0.0' }
        }
        break
      }
      ...
      case 'tools/list': {
        result = {
          tools: monitoringDefinition.tools.map((tool) => ({
            name: tool.name,
      ...
      case 'tools/call': {
        ...
        const tool = monitoringDefinition.tools.find((t) => t.name === toolName)
        if (!tool) {
    ...
    console.error('[BuiltinMonitoringMCP] Error handling request:', error)
    return {
      jsonrpc: '2.0',
      id: requestId,
      error: {
