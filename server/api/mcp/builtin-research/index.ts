...
export default defineEventHandler(async (event) => {
  ...
  if (method === 'GET') {
    return {
      ok: true,
      message: 'Builtin Research MCP endpoint. Send JSON-RPC 2.0 requests via POST.'
    }
  }
  ...
  console.log('[BuiltinResearchMCP] Authenticated:', {
    teamId,
    userId,
  ...
  const baseContext: ResearchMcpContext = {
    teamId,
    userId
  }
  ...
  try {
    ...
    switch (body.method) {
      case 'initialize': {
        result = {
          ...
          capabilities: { tools: {} },
          serverInfo: { name: 'builtin-research-server', version: '1.0.0' }
        }
        break
      }
      ...
      case 'tools/list': {
        result = {
          tools: researchDefinition.tools.map((tool) => ({
            name: tool.name,
      ...
      case 'tools/call': {
        ...
        const tool = researchDefinition.tools.find((t) => t.name === toolName)
        if (!tool) {
    ...
    console.error('[BuiltinResearchMCP] Error handling request:', error)
    return {
      jsonrpc: '2.0',
      id: requestId,
      error: {
