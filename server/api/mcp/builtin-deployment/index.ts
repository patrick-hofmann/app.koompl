...
export default defineEventHandler(async (event) => {
  ...
  if (method === 'GET') {
    return {
      ok: true,
      message: 'Builtin Deployment MCP endpoint. Send JSON-RPC 2.0 requests via POST.'
    }
  }
  ...
  console.log('[BuiltinDeploymentMCP] Authenticated:', {
    teamId,
    userId,
  ...
  const baseContext: DeploymentMcpContext = {
    teamId,
    userId
  }
  ...
  try {
    ...
    switch (body.method) {
      case 'initialize': {
        result = {
          ...
          capabilities: { tools: {} },
          serverInfo: { name: 'builtin-deployment-server', version: '1.0.0' }
        }
        break
      }
      ...
      case 'tools/list': {
        result = {
          tools: deploymentDefinition.tools.map((tool) => ({
            name: tool.name,
      ...
      case 'tools/call': {
        ...
        const tool = deploymentDefinition.tools.find((t) => t.name === toolName)
        if (!tool) {
    ...
    console.error('[BuiltinDeploymentMCP] Error handling request:', error)
    return {
      jsonrpc: '2.0',
      id: requestId,
      error: {
