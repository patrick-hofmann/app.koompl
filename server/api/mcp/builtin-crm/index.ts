/**
 * Builtin CRM MCP Server Endpoint
 *
 * Accepts MCP JSON-RPC 2.0 requests over HTTP and surfaces CRM tools.
 */
...
import { crmDefinition } from './definition'
...
export default defineEventHandler(async (event) => {
  ...
  if (method === 'GET') {
    return {
      ok: true,
      message: 'Builtin CRM MCP endpoint. Send JSON-RPC 2.0 requests via POST.'
    }
  }
  ...
  console.log('[BuiltinCrmMCP] Authenticated:', {
    teamId,
    userId,
  ...
  if (hasCookies) {
    ...
    if (session) {
      ...
      console.log('[BuiltinCrmMCP] Authenticated via session:', { teamId, userId })
    } else {
      // No valid session - fall through to header auth
      console.log('[BuiltinCrmMCP] No valid session, using header auth')
    }
  }

  if (!session) {
    // No session (webhook call) - trust headers (they're set by our own code)
    console.log('[BuiltinCrmMCP] Authenticated via headers (webhook):', {
      teamId,
      userId,
      hasCookies
    })
  ...
  const baseContext: CrmMcpContext = {
    teamId,
    userId,
  ...
  try {
    ...
    switch (body.method) {
      case 'initialize': {
        ...
        console.log(`[${timestamp}] [BuiltinCrmMCP] Method: initialize`)
        result = {
      ...
      case 'tools/list': {
        ...
        const timestamp = new Date().toISOString()
        console.log(`[${timestamp}] [BuiltinCrmMCP] Method: tools/list`)
        result = {
          tools: crmDefinition.tools.map((tool) => ({
            name: tool.name,
        ...
        console.log(
          `[${endTimestamp}] [BuiltinCrmMCP] ⏱️  tools/list completed in ${duration}ms (${crmDefinition.tools.length} tools)`
        )
        break
      }
      ...
      case 'tools/call': {
        ...
        const timestamp = new Date().toISOString()
        console.log(`[${timestamp}] [BuiltinCrmMCP] Tool called: ${toolName}`, { args })

        const allowOverride =
          process.env.NODE_ENV === 'development' ||
          getRequestHeader(event, 'x-mcp-allow-team-override') === '1'
        const resolvedContext: CrmMcpContext = {
          ...baseContext,
          teamId:
        ...
        const tool = crmDefinition.tools.find((t) => t.name === toolName)
        if (!tool) {
        ...
        console.log(`[BuiltinCrmMCP] ⏱️  Tool '${toolName}' completed in ${toolDuration}ms`)
    ...
    console.error('[BuiltinCrmMCP] Error handling request:', error)
    return {
      jsonrpc: '2.0',
      id: requestId,
      error: {
