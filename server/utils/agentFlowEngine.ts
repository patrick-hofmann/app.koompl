...
export class AgentFlowEngine {
   ...
   */
  async listAgentFlows(agentId: string, filters?: {
    status?: FlowStatus[]
    limit?: number
  }): Promise<AgentFlow[]> {
    const flows = await this.listFlows({
   ...
   */
    ...
    console.log(`[AgentFlowEngine] │  Reasoning: ${decision.reasoning.substring(0, 100)}${decision.reasoning.length > 100 ? '...' : ''}`)
    switch (decision.type) {
      case 'continue':
        // Continue to next round - check if we haven't exceeded max rounds
        if (flow.currentRound >= flow.maxRounds) {
          console.log(`[AgentFlowEngine] ⚠ Max rounds (${flow.maxRounds}) reached, completing flow`)
          await this.completeFlow(flow.id, 'Flow completed after reaching maximum rounds', flow.agentId)
          return {
            decision: 'complete',
  ...
  private async handleWaitForAgent(flow: AgentFlow, decision: FlowDecision): Promise<void> {
    ...
    console.log(`[AgentFlowEngine] 📧 Subject: [Req: ${requestId}] ${decision.targetAgent.messageSubject}`)
    console.log(`[AgentFlowEngine] ⏳ Flow ${flow.id} now waiting for response from agent ${toAgentEmail}`)
